...
async function versions(mc) {
  ...
  if (isOldGroup(mc)) {
    ...
    const snapMatch = /-snapshot-(\d+)$/.exec(mc)
    const preMatch = /-pre-(\d+)$/.exec(mc)
    if (snapMatch) {
      const re = new RegExp('\\+snapshot-' + snapMatch[1] + '$')
      list = all.filter((v) => re.test(v))
    } else if (preMatch) {
      const re = new RegExp('\\+pre-' + preMatch[1] + '$')
      list = all.filter((v) => re.test(v))
    } else {
      const parts = String(mc).split('.')
      const base = String(mc).replace(/^1\./, '')
      const prefix = base + (parts.length === 2 ? '.0.' : '.')
      list = all.filter((v) => v.startsWith(prefix) && !v.includes('+'))
    }
  }
...
async function gameVersions() {
  ...
  try {
    ...
    for (const v of parseVersions(xml)) {
      const m = /^(\d+)\.(\d+)\./.exec(v)
      if (m) {
        const major = parseInt(m[1], 10)
        const minor = parseInt(m[2], 10)
        if (major >= 26) {
          const patch = parseInt((/^(\d+)\.(\d+)\.(\d+)\./.exec(v) || [])[3], 10)
          if (patch > 0) mcs.add(major + '.' + minor + '.' + patch)
          else mcs.add(major + '.' + minor)
        } else if (major >= 20) {
          mcs.add('1.' + major + '.' + minor)
          if (minor === 0) mcs.add('1.' + major)
        }
      }
      const sm = /\+snapshot-(\d+)/.exec(v)
      if (sm) {
        const b = /^(\d+)\.(\d+)\./.exec(v)
        if (b && parseInt(b[1], 10) >= 26) mcs.add(b[1] + '.' + b[2] + '-snapshot-' + sm[1])
      }
      const pm = /\+pre-(\d+)/.exec(v)
      if (pm) {
        const b = /^(\d+)\.(\d+)\./.exec(v)
        if (b && parseInt(b[1], 10) >= 26) mcs.add(b[1] + '.' + b[2] + '-pre-' + pm[1])
      }
    }
  } catch (e) {}
  try {
    const xml2 = await getText(NEOFORGE_FORGE_MAVEN + 'maven-metadata.xml')
    for (const v of parseVersions(xml2)) {
...
module.exports = { versions, installerUrl, ensureInstalled, findInstalled, gameVersions }
