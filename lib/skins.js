...
function multipart(fields, fileBuffer, filename) {
  const boundary = '----KrypLauncher' + Date.now() + Math.random().toString(16).slice(2)
  const parts = []
...
function packMetaFor(versionId) {
  ...
  if (fmt === null || fmt < 65) {
    return JSON.stringify({ pack: { pack_format: fmt === null ? 15 : fmt, description: 'KrypLauncher Skin' } })
  }
  return JSON.stringify({ pack: { min_format: fmt, max_format: 999, description: 'KrypLauncher Skin' } })
}
...
function installOfflineSkin(gameDir, pngBuffer, versionId) {
  ...
  const zipFile = path.join(rpDir, 'KrypLauncherSkin.zip')
  ...
  const packName = 'file/KrypLauncherSkin.zip'
...
function removeOfflineSkinPack(gameDir) {
  const zipFile = path.join(gameDir, 'resourcepacks', 'KrypLauncherSkin.zip')
  ...
  for (const key of ['resourcePacks', 'incompatibleResourcePacks']) {
    let arr = []
    try { arr = JSON.parse(lines[key] || '[]') } catch (e) {}
    const next = arr.filter((x) => x !== 'file/KrypLauncherSkin.zip')
